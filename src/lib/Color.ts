...
export default class Color{
    ...
    set hex(data){
        const hex: string = data in _colorKeywords ? _colorKeywords[data] : data
        const hexReg = hex.replace(
            /^#([a-f\d])([a-f\d])([a-f\d])$/i ,(_, r, g, b) => 
            '#' + r + r + g + g + b + b).substring(1).match(/.{2}/g)
        const rgb = hexReg === null ? 
            [0,0,0] : 
    ...
    mix(other: Color, proportion = 0.5) {
        const mix_channel = (a:number, b:number) => a*(1-proportion) + b*proportion
        const c: triple = [
